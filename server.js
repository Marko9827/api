...
const http2 = require("http"),
    ...
    httpErrorPages = require('http-error-pages'),
...
"><svg id=logo aria-hidden="true" focusable="false"
  ...
  <p><b style="color:#607D8B;">${code}.</b> <ins>${text_title}</ins>
  <p>The requested URL <code style="
    color: rgb(255 0 0 / 72%);
">${url2}</code>${text_content}<br><button onclick="window.location.href = '/';" title="Go Home page">Go Home</button>
`;
};
...
try {
   ...
   app.use(function(err,req,res,next){
     res.status(404).send(error_page(404, "That’s an error.", " Was not found on this server.  <ins> That’s all we know. </ins>", url));
     res.status(200).send(console.info("\n" + chalk.green("=>") + " Successfully loaded (" + chalk.green(200) + ") : " + chalk.green(url) + " | Time: " + time2()));
    ...
    app.get('*', function (req, res) {
       ...
       if (res.statusCode == 200) {
         ...
         res.status(200).send(res.render()+console.info("\n" + chalk.green("=>") + " Successfully loaded (" + chalk.green(200) + ") : " + chalk.green(url) + " | Time: " + time2()));
       } */
...
var server_credits = "" + chalk.cyan("Made the entire source code of the app: ") + chalk.bgBlackBright("Marko Nikolic") + "\n" +
    "Linkedin : " + chalk.greenBright(LINKEDIN) + " <3 \n" +
    "Github   : " + chalk.greenBright(GITHUB) + " <3";
var server = app.listen(PORT, function () {
    server.timeout = 2000000;
    var host = server.address().address;
    var port = server.address().port;
    console.log("\n" + chalk.yellow("Eronelit Edroid server") + " running at \n\n=> " + chalk.greenBright("http") + "://" + chalk.magenta("localhost") + ":" + chalk.magentaBright(port) +
        "/ \n=> " + chalk.gray("Protocol") + ": " + chalk.greenBright("http") + "\n=> " + chalk.gray("Host") + ": " + chalk.magenta(host) +
        "\n=> " + chalk.gray("Port") + ": " + chalk.magentaBright(port) +
        "\n=> " + chalk.gray("Root") + ": " + chalk.blueBright(__dirname) +
        "\n=> " + chalk.gray("Brain port") + ": " + chalk.greenBright("49510:5091:V:041") + "\n");
    console.log("\x1b[36m%s\x1b[0m", server_credits);


    console.log("\nFor exit " + chalk.red("server") + " using key commands \n=> " + chalk.bgGrey("ctrl") + " " + chalk.blue("+") + " " + chalk.bgGrey("c"));
});
